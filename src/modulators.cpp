...
LfoSource::LfoSource(
    uint32_t channel, Shape shape, float rate_beats, float depth, float offset) noexcept
    : channel_{channel}
{
...
void EnvelopeSource::advance(uint64_t to_tick, EventDispatcher& /*dispatcher*/, ProcessContext& ctx)
{
    ...
    auto tick_f = static_cast<float>(to_tick);

    if (loop_ && count_ > 1u)
    {
        const auto period = static_cast<float>(points_[count_ - 1u].tick);
        if (period > 0.0f)
        {
    ...
    {
        const auto t0 = static_cast<float>(points_[i].tick);
        const auto t1 = static_cast<float>(points_[i + 1u].tick);
...
StepModulatorSource::StepModulatorSource(uint32_t channel, uint64_t step_ticks, bool loop) noexcept
...
void StepModulatorSource::advance(uint64_t to_tick,
