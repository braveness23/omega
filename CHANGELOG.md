...
- **`omega::ControlSink`** (`include/omega/control_sink.h`): custom `OutputSink` that intercepts `OMEGA_CTRL_*` payload events and executes them as direct mutations on the timing thread (`perf_.cue/stop/set_transpose`, `tempo_.insert`). Control patterns route events here by using `ctrl_sink.sink_id()` as their events' `sink_id`. Thread-safe: same thread as `Engine::apply()` overloads.
...
- **`Engine::perf_source()` non-const accessor**: parallel to existing `timeline_source()` and `tempo_map()` non-const overloads; required for `ControlSink` construction.
