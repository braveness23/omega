...
TEST_CASE("F8: copy_events returns only the events inside [lo, hi)")
{
    ...
    omega_event_t buf[8];
    omega_event_id_t ids[8];
    size_t total = 0;
    // Window [0, 700) covers ticks 0 and 480 but not 960.
    omega_status_t st =
        omega_engine_track_copy_events(fx.e, fx.track, 0u, 700u, 0xFFu, buf, ids, 8u, &total);
    REQUIRE(st == OMEGA_OK);
    REQUIRE(total == 2u);
...
TEST_CASE("F9: replace_event_by_id moves a note while keeping its id")
{
    ...
    omega_event_id_t ids[8];
    size_t total = 0;
    omega_engine_track_copy_events(fx.e, fx.track, 0u, 10000u, 0xFFu, buf, ids, 8u, &total);
    REQUIRE(total == 2u);
...
TEST_CASE("F9: replace_event_by_id then undo restores the original")
{
    Fixture fx{0u};
    omega_event_t buf[4];
    omega_event_id_t ids[4];
    size_t total = 0;
    omega_engine_track_copy_events(fx.e, fx.track, 0u, 10000u, 0xFFu, buf, ids, 4u, &total);
